...
function TranslatorApp({ onClose }) {
 ...
 const translateWithClaude = async (text, targetCode) => {
  ...
  const prompt = `You are a professional translator. The following text may be Roman Urdu (Urdu written in Latin script, e.g. "Aap ka naam kya hai?") OR Urdu script (e.g. "آپ کا نام کیا ہے"). Detect which it is and translate it into ${targetName}.
...
${text}`;
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    headers: {
      ...
      // Add your API key here or use your proxy
    },
    body: JSON.stringify({
  ...
  const data = await response.json();
  const textBlock = data.content?.find((b) => b.type === "text");
  if (!textBlock?.text) throw new Error("No translation returned");
